    ...
    <>
      ...
      <section className="bg-[#0D0C18] px-6 md:px-14 py-20">
        <div className="max-w-6xl mx-auto space-y-8">
          {featured && (
            <Reveal>
              <div className="bg-white/4 border border-white/8 rounded-3xl overflow-hidden grid grid-cols-1 lg:grid-cols-[1fr_0.7fr]">
                <div className="p-8 md:p-10">
                  ...
                  <p className="text-base text-white/60 leading-relaxed mb-6">{featured.description}</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                    {[
                      ...
                      <div key={row.label}>
                        ...
                        <span className="text-sm text-white/75">{row.value}</span>
                      </div>
                    ))}
                  </div>
                  <div className="flex flex-wrap gap-2 mb-8">
                    {featured.highlights.map((h) => (
                      <span key={h} className="text-xs text-white/60 bg-white/6 border border-white/10 px-3 py-1.5 rounded-full">{h}</span>
                    ))}
                  </div>
                  <div className="flex flex-wrap gap-4">
                    ...
                    <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="inline-flex bg-white/8 border border-white/20 text-white font-semibold text-sm px-8 py-4 rounded-full hover:bg-white/15 transition-all font-sans">
                      WhatsApp AkiliNest
                    </a>
                  </div>
                </div>
          ...
          <Reveal delay={80}>
            <div className="bg-white/4 border border-white/8 rounded-3xl p-8 md:p-10 mb-10">
              <span className="inline-block bg-[#7F5AF0]/20 text-[#7BCFE8] text-xs font-bold tracking-[1.5px] uppercase px-4 py-2 rounded-full mb-5">
                Holiday Camps
              </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
                {holidayCampsFlyer.intakes.map((intake) => (
                  <div key={intake.dates} className="bg-white/6 border border-white/10 rounded-xl px-4 py-3">
                    <span className="text-[10px] font-bold uppercase tracking-wide text-[#7BCFE8] block mb-1">{intake.month}</span>
                    <span className="text-sm text-white/80">{intake.dates}</span>
                  </div>
                ))}
              </div>
              <div className="flex flex-wrap gap-2 mb-8">
                {[holidayCampsFlyer.time, holidayCampsFlyer.duration, ...holidayCampsFlyer.highlights].map((item) => (
                  <span key={item} className="text-xs text-white/60 bg-white/6 border border-white/10 px-3 py-1.5 rounded-full">
                    {item}
                  </span>
                ))}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {camps.map((camp) => (
                <div key={camp.id} className="bg-white/4 border border-white/8 rounded-2xl p-6 hover:border-white/15 transition-colors">
                  ...
                  <p className="text-sm text-white/50 mb-4">{camp.time}{camp.duration ? ` · ${camp.duration}` : ""}</p>
                  <p className="text-sm text-white/55 leading-relaxed mb-4">{camp.description}</p>
          ...
          <Reveal delay={120}>
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_0.8fr] gap-6 bg-white/3 border border-white/6 rounded-2xl overflow-hidden">
              <div className="p-6">
                <h3 className="font-serif text-lg font-bold text-white mb-2">The venue</h3>
                <p className="text-sm text-white/55 leading-relaxed mb-2">{VENUE}</p>
                <p className="text-sm text-white/45">Teal walls, parquet floors, children&apos;s art gallery, fairy lights. Intimate and hands-on.</p>
              </div>
          ...
          <Reveal delay={140}>
            <div className="bg-white/3 border border-white/6 rounded-2xl p-6">
              ...
              <p className="text-sm text-white/45 leading-relaxed mb-4">Weekend sessions across all four stages. Join the early access list to be first to know.</p>
